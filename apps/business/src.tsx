...
function App() {
  ...
  useEffect(() => {
    ...
    load().catch((error: Error) => { setMessage(error.message); setLoading(false); });
  }, []);
  ...
  return <Shell product="Business" demoRole="BUSINESS_MANAGER">
    ...
    <section className="split">
      <article className="panel"><span className="eyebrow">Budget utilisation</span><h2>Monthly spending</h2>
        <div className="chart" aria-label="Six month employee travel spending">
          {[46,58,51,73,68,82].map((height,index) => <span key={index} style={{height:`${height}%`}} data-label={["Feb","Mar","Apr","May","Jun","Jul"][index]} />)}
        </div>
      </article>
      <article className="panel"><span className="eyebrow">Billing summary</span><h2>July 2026</h2>
        <div className="mini-row"><span>Employee rides</span><strong>{account?.currentMonth.rides ?? 0}</strong></div>
        <div className="mini-row"><span>Current charges</span><strong>{money(account?.currentMonth.spendMinor ?? 0)}</strong></div>
        <div className="mini-row"><span>Payment terms</span><strong>Net 30</strong></div>
        <button className="action secondary" onClick={() => window.print()}>Download invoice</button>
      </article>
    </section>
    ...
    <section className="split">
      <article className="panel"><span className="eyebrow">Approval rules</span><h2>Department controls</h2>
        <div className="mini-row"><span>Operations</span><strong>Auto-approve · LRD 6,000</strong></div>
        <div className="mini-row"><span>Finance</span><strong>Weekdays · LRD 4,500</strong></div>
        <div className="mini-row"><span>Field teams</span><strong>24/7 · LRD 8,000</strong></div>
        <button className="action secondary">Edit ride policy</button>
      </article>
      <article className="panel"><span className="eyebrow">Usage reports</span><h2>Travel insights</h2>
        <div className="mini-row"><span>Most used route</span><strong>Sinkor → Broad Street</strong></div>
        <div className="mini-row"><span>Average trip</span><strong>LRD 920</strong></div>
        <div className="mini-row"><span>On-policy rides</span><strong>96%</strong></div>
        <button className="action secondary">Export CSV report</button>
      </article>
    </section>
    <section className="panel"><span className="eyebrow">Recent travel</span><h2>Employee trip history</h2>
      <table><thead><tr><th>Employee</th><th>Route</th><th>Department</th><th>Fare</th><th>Status</th></tr></thead><tbody>
        {["Miatta Kamara","Samuel Doe","Hawa Kollie","Josephine Wreh"].map((name,index)=><tr key={name}><td>{name}</td><td>{index%2 ? "ELWA → Sinkor" : "Paynesville → Broad Street"}</td><td>{["Operations","Finance","Field","Leadership"][index]}</td><td>{money(78_000+index*9_000)}</td><td>Completed</td></tr>)}
      </tbody></table>
    </section>
  </Shell>;
}
