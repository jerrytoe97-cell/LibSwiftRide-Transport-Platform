...
For each staging drill, record the source recovery point, target isolated database, operator, timestamps and checksums. Restore the provider-managed snapshot or PITR point, then run `pnpm db:deploy` against the isolated target. Compare counts and totals for `Ride`, `Payment`, `Refund`, `WalletTransaction`, `DriverPayout` and `AuditLog`; verify every new completed ride has a balanced 86/14 allocation and that wallet and first-ride 2% referral transactions reconcile. Exercise `/health/ready` and a sandbox booking before declaring the restore usable. Never overwrite the source database during a drill.

Point `DATABASE_URL` only at the isolated restored database and run `pnpm recovery:validate`. The command emits sanitized counts, aggregate minor-unit totals and violation counts; it exits nonzero for an invalid completed-ride allocation, a wallet whose stored balance differs from its latest transaction balance, or refunds exceeding their payment. The output contains no customer identity, KYC reference or location data and should be attached to the restore evidence.
