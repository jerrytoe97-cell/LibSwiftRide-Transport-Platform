...
describe("ride state machine", () => {
  it("allows the normal trip lifecycle", () => {
    expect(canTransition("DRIVER_ARRIVED", "PASSENGER_BOARDED")).toBe(true);
    expect(canTransition("PASSENGER_BOARDED", "IN_PROGRESS")).toBe(true);
    expect(canTransition("IN_PROGRESS", "COMPLETED")).toBe(true);
  });
  it("blocks skipping required safety states", () => {
    expect(() => assertTransition("SEARCHING", "COMPLETED")).toThrow();
    expect(() => assertTransition("DRIVER_ARRIVED", "IN_PROGRESS")).toThrow();
  });
});
