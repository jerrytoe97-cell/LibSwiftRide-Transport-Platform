...
Deploy `render.kyc-scanner.yaml` as a separate Render Blueprint/service. Use an always-on plan with at least 2 GiB RAM because current ClamAV databases approach 1 GiB resident memory while loading; a sleeping or memory-constrained scanner would cause scans to fail closed. The scanner image pins the official `clamav/clamav-debian:1.4.6` release, which includes a signed database snapshot, and `freshclam` applies current updates at container startup. Docker provides a 14-minute startup grace while `/health/ready` continues to fail closed until `clamd` can actually scan. Render allows a new service deployment up to 15 minutes to pass its configured `/health/ready` check. Monitor engine support, image rebuilds, signature age, startup duration, and failed `freshclam` updates; update the pinned release deliberately after verification rather than tracking a mutable tag.
